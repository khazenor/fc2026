ServerEvents.recipes(event => {
  event.shapeless(
    Item.of('minecraft:allium', 8),
    ['minecraft:allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:azure_bluet', 8),
    ['minecraft:azure_bluet', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:blue_orchid', 8),
    ['minecraft:blue_orchid', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:cornflower', 8),
    ['minecraft:cornflower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:dandelion', 8),
    ['minecraft:dandelion', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:lilac', 8),
    ['minecraft:lilac', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:lily_of_the_valley', 8),
    ['minecraft:lily_of_the_valley', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:orange_tulip', 8),
    ['minecraft:orange_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:oxeye_daisy', 8),
    ['minecraft:oxeye_daisy', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:peony', 8),
    ['minecraft:peony', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:pink_tulip', 8),
    ['minecraft:pink_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:poppy', 8),
    ['minecraft:poppy', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:red_tulip', 8),
    ['minecraft:red_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:rose_bush', 8),
    ['minecraft:rose_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:sunflower', 8),
    ['minecraft:sunflower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:white_tulip', 8),
    ['minecraft:white_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:wither_rose', 8),
    ['minecraft:wither_rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:torchflower_seeds', 8),
    ['minecraft:torchflower_seeds', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:flowering_azalea', 8),
    ['minecraft:flowering_azalea', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:moss_block', 8),
    ['minecraft:moss_block', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:moss_carpet', 8),
    ['minecraft:moss_carpet', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:pink_petals', 8),
    ['minecraft:pink_petals', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:spore_blossom', 8),
    ['minecraft:spore_blossom', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:big_dripleaf', 8),
    ['minecraft:big_dripleaf', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:sea_pickle', 8),
    ['minecraft:sea_pickle', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:lily_pad', 8),
    ['minecraft:lily_pad', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:vine', 8),
    ['minecraft:vine', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('minecraft:glow_lichen', 8),
    ['minecraft:glow_lichen', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:jacaranda_bush', 8),
    ['biomeswevegone:jacaranda_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:blue_rose_bush', 8),
    ['biomeswevegone:blue_rose_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:hydrangea_bush', 8),
    ['biomeswevegone:hydrangea_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:firecracker_flower_bush', 8),
    ['biomeswevegone:firecracker_flower_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:clover_patch', 8),
    ['biomeswevegone:clover_patch', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:flower_patch', 8),
    ['biomeswevegone:flower_patch', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:leaf_pile', 8),
    ['biomeswevegone:leaf_pile', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:water_silk', 8),
    ['biomeswevegone:water_silk', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:white_sakura_petals', 8),
    ['biomeswevegone:white_sakura_petals', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:yellow_sakura_petals', 8),
    ['biomeswevegone:yellow_sakura_petals', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:alpine_bellflower', 8),
    ['biomeswevegone:alpine_bellflower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:amaranth', 8),
    ['biomeswevegone:amaranth', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:angelica', 8),
    ['biomeswevegone:angelica', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:bistort', 8),
    ['biomeswevegone:bistort', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:black_rose', 8),
    ['biomeswevegone:black_rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:california_poppy', 8),
    ['biomeswevegone:california_poppy', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:crocus', 8),
    ['biomeswevegone:crocus', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:cyan_amaranth', 8),
    ['biomeswevegone:cyan_amaranth', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:cyan_rose', 8),
    ['biomeswevegone:cyan_rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:delphinium', 8),
    ['biomeswevegone:delphinium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:fairy_slipper', 8),
    ['biomeswevegone:fairy_slipper', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:foxglove', 8),
    ['biomeswevegone:foxglove', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:iris', 8),
    ['biomeswevegone:iris', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:japanese_orchid', 8),
    ['biomeswevegone:japanese_orchid', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:kovan_flower', 8),
    ['biomeswevegone:kovan_flower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:lazarus_bellflower', 8),
    ['biomeswevegone:lazarus_bellflower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:lollipop_flower', 8),
    ['biomeswevegone:lollipop_flower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:magenta_amaranth', 8),
    ['biomeswevegone:magenta_amaranth', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:orange_amaranth', 8),
    ['biomeswevegone:orange_amaranth', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:orange_daisy', 8),
    ['biomeswevegone:orange_daisy', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:peach_leather_flower', 8),
    ['biomeswevegone:peach_leather_flower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:pink_anemone', 8),
    ['biomeswevegone:pink_anemone', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:pink_daffodil', 8),
    ['biomeswevegone:pink_daffodil', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:protea_flower', 8),
    ['biomeswevegone:protea_flower', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:purple_amaranth', 8),
    ['biomeswevegone:purple_amaranth', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:rose', 8),
    ['biomeswevegone:rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:snowdrops', 8),
    ['biomeswevegone:snowdrops', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:winter_cyclamen', 8),
    ['biomeswevegone:winter_cyclamen', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:winter_rose', 8),
    ['biomeswevegone:winter_rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:winter_scilla', 8),
    ['biomeswevegone:winter_scilla', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:winter_succulent', 8),
    ['biomeswevegone:winter_succulent', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:yellow_daffodil', 8),
    ['biomeswevegone:yellow_daffodil', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:alpine_poppy', 8),
    ['meadow:alpine_poppy', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:delphinium', 8),
    ['meadow:delphinium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:saxifrage', 8),
    ['meadow:saxifrage', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:enzian', 8),
    ['meadow:enzian', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:fire_lily', 8),
    ['meadow:fire_lily', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:eriophorum', 8),
    ['meadow:eriophorum', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:tall_eriophorum', 8),
    ['meadow:tall_eriophorum', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('candlelight:rose', 8),
    ['candlelight:rose', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('herbalbrews:lavender', 8),
    ['herbalbrews:lavender', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('herbalbrews:hibiscus', 8),
    ['herbalbrews:hibiscus', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:pink_allium', 8),
    ['biomeswevegone:pink_allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:white_allium', 8),
    ['biomeswevegone:white_allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:tall_allium', 8),
    ['biomeswevegone:tall_allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:tall_pink_allium', 8),
    ['biomeswevegone:tall_pink_allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:tall_white_allium', 8),
    ['biomeswevegone:tall_white_allium', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:allium_flower_bush', 8),
    ['biomeswevegone:allium_flower_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:pink_allium_flower_bush', 8),
    ['biomeswevegone:pink_allium_flower_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:white_allium_flower_bush', 8),
    ['biomeswevegone:white_allium_flower_bush', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:cyan_tulip', 8),
    ['biomeswevegone:cyan_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:green_tulip', 8),
    ['biomeswevegone:green_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:magenta_tulip', 8),
    ['biomeswevegone:magenta_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:purple_tulip', 8),
    ['biomeswevegone:purple_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:yellow_tulip', 8),
    ['biomeswevegone:yellow_tulip', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:beach_grass', 8),
    ['biomeswevegone:beach_grass', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:cattail_sprout', 8),
    ['biomeswevegone:cattail_sprout', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:fluorescent_cattail_sprout', 8),
    ['biomeswevegone:fluorescent_cattail_sprout', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:green_mushroom', 8),
    ['biomeswevegone:green_mushroom', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:prairie_grass', 8),
    ['biomeswevegone:prairie_grass', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:shrub', 8),
    ['biomeswevegone:shrub', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:white_puffball_spores', 8),
    ['biomeswevegone:white_puffball_spores', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('meadow:small_fir', 8),
    ['meadow:small_fir', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('yungscavebiomes:prickly_peach_cactus', 8),
    ['yungscavebiomes:prickly_peach_cactus', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('yungscavebiomes:prickly_vines', 8),
    ['yungscavebiomes:prickly_vines', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('yungscavebiomes:frost_lily', 8),
    ['yungscavebiomes:frost_lily', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:green_mushroom', 8),
    ['biomeswevegone:green_mushroom', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:shelf_fungi', 8),
    ['biomeswevegone:shelf_fungi', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:weeping_milkcap', 8),
    ['biomeswevegone:weeping_milkcap', 'minecraft:bone_meal'])
  event.shapeless(
    Item.of('biomeswevegone:wood_blewit', 8),
    ['biomeswevegone:wood_blewit', 'minecraft:bone_meal'])

})